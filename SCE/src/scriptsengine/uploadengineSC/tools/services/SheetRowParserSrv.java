...
public class SheetRowParserSrv implements ISheetRowParserSrv
{
	...
	private Row getRowforFieldName(XSSFSheet sheetRef, String fldName) throws EX_General
	{
		Row rowVals = null;
		Row rowCurr = null;
	...
	private <T> ArrayList<T> getValsforRow(XSSFSheet sheetRef, Row rowVals, FieldsParserProp parserProp) throws EX_General
	{
		...
		if (cellIterator != null && wbMdtSrv != null)
		{
			...
			while (cellIterator.hasNext())
			{
				...
				if (colpos >= (parserProp.getColStartPos() - 1) && colpos <= (parserProp.getColEndPos() - 1))
				{
					if (cell != null)
					{
						SheetFieldsMetadata fldMdt = wbMdtSrv.getFieldMetadata(sheetRef.getSheetName(), parserProp.getSheetField());
						if (fldMdt != null)
						{
