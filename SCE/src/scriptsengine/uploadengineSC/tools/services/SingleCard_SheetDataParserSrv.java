...
import scriptsengine.uploadengineSC.tools.definitions.TY_AttribVal;
import scriptsengine.uploadengineSC.tools.definitions.TY_SingleCard_SheetRawData;
import scriptsengine.uploadengineSC.tools.interfaces.ISingleCard_SheetDataParserSrv;
...
public class SingleCard_SheetDataParserSrv implements ISingleCard_SheetDataParserSrv
{
	...
	private ArrayList<TY_AttribVal> getFldVals(XSSFSheet sheetRef, SCSheetMetadata shMdt) throws EX_General
	{
				...
				while ((rowCurr = rowIterator.next()) != null)
				{
					...
					if (cellOne != null)
					{
						...
						if (fldNametoCmp != null)
						{
							...
							try
							{
								SheetFieldsMetadata fldMdt = shMdt.getFldsMdt().stream().filter(x -> x.getSheetField().equals(fldNametoCmp))
								          .findFirst().get();
								if (fldMdt != null)
								{
									TY_AttribVal attrVal = new TY_AttribVal(fldMdt.getObjField(), fldNametoCmp, null, fldMdt.getDataType());
	...
	private Object getValueforField(SheetFieldsMetadata fldMdt, Cell cell, FormulaEvaluator eval) throws EX_General
	{
		Object val = null;
