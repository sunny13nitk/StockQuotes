...
public class WBRawDataSrv implements IWBRawDataSrv
{
	...
	private SheetFldValsHeadersList getCollSheetFldVals(XSSFSheet sheetRef) throws EX_General
	{
						...
						if (headers != null)
						{
							if (headers.size() > 0)
							{
								...
								for ( SheetFieldsMetadata fldMdt : shMdt.getFldsMdt() )
								{
									...
									if (shMdt.getHeadScanConfig().getColToScanStart() > shtHdrSrv.getNonBlankColPosBegin())
									{
	...
	private TY_SingleCard_SheetRawData getNonCollSheetFldVals(XSSFSheet sheetRef) throws EX_General
	{
