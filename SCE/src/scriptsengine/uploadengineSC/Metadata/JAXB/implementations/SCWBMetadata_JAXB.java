package scriptsengine.uploadengineSC.Metadata.JAXB.implementations;
...
import scriptsengine.enums.SCEenums.rowDataType;
import scriptsengine.enums.SCEenums.rowScanType;
...
import scriptsengine.uploadengineSC.tools.definitions.EntityListHeadScannerConfig;
...
public class SCWBMetadata_JAXB implements IJaxable
{
	...
	public void setPathConstants(PathsJAXBSM pathConstants)
	{
	...
	public SCWBMetadata_JAXB(PathsJAXBSM pathConstants)
	{
		super();
		this.pathConstants = pathConstants;
	}
	...
	public void Generate_XML() throws Exception
	{
		...
		EntityListHeadScannerConfig anHeadConfig = new EntityListHeadScannerConfig(2, 2, 11, rowScanType.Continous, rowDataType.String,
		          rowDataType.Numerical, true, 3, "Analysis_HeadersConvSrv", "Year", rowDataType.Numerical);
		sheetAnalysis.setHeadScanConfig(anHeadConfig);

		ArrayList<SheetFieldsMetadata> sheetFlds = new ArrayList<SheetFieldsMetadata>();
		if (sheetFlds != null)
		{
			SheetFieldsMetadata salesfldMdt = new SheetFieldsMetadata("Sales", "sales", true, false, false, false, false, rowDataType.Numerical);
			sheetFlds.add(salesfldMdt);
			SheetFieldsMetadata opfldMdt = new SheetFieldsMetadata("Operating Profit", "op", true, false, false, false, false,
			          rowDataType.Numerical);
			sheetFlds.add(opfldMdt);
			SheetFieldsMetadata opmfldMdt = new SheetFieldsMetadata("Operating Profit Margin (OPM%)", "opm", true, false, false, false, false,
			          rowDataType.Numerical);
		...
		EntityListHeadScannerConfig trHeadConfig = new EntityListHeadScannerConfig(1, 2, 6, rowScanType.Continous, rowDataType.String,
		          rowDataType.Numerical, true, 2, null, "period", rowDataType.String);
	...
	public <T> ArrayList<T> Load_XML_to_Objects() throws Exception
	{
		...
		try
		{
			...
			InputStream is = this.getClass().getClassLoader().getResourceAsStream(this.getPathConstants().getJaxPath_SM_xml());
