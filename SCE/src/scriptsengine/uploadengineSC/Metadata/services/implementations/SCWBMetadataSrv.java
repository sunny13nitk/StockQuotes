package scriptsengine.uploadengineSC.Metadata.services.implementations;
...
public class SCWBMetadataSrv implements ISCWBMetadataSrv
{
	...
	public SCSheetMetadata getMetadataforSheet(String SheetName) throws EX_General
	{
		...
		if (SheetName != null)
		{
			...
			try
			{
				shtMdt = this.wbMdt.getSheetMetadata().stream().filter(x -> x.getSheetName().equals(SheetName)).findFirst().get();
			}
	...
	public BaseSheetKey getBaseSheetKey() throws EX_General
	{
		if (this.bshKey == null)
		{
			...
			try
			{
				SCSheetMetadata shtMdt = this.wbMdt.getSheetMetadata().stream().filter(x -> x.isBaseSheet() == true).findFirst().get();
				if (shtMdt != null)
				{
					SheetFieldsMetadata fldMdt = shtMdt.getFldsMdt().stream().filter(x -> x.isKey() == true).findFirst().get();
					if (fldMdt != null)
					{
	...
	private void Initialize() throws EX_General
	{
		if (pathsSMbean != null)
		{
