...
/**
 * 
 * Ratio of Effective Dividend per Share over years i.e Dividend/Face Value of share in a particular Year
 */
@Service("KFG_EffDPS_Srv")
public class KFG_EffDPS_Srv implements IKeyFiguresGenerator
{
					...
					if (balSheetEntList.size() > 0)
					{
						for ( OB_Scrip_BalSheet ob_Scrip_BalSheet : balSheetEntList )
						{
							TY_KeyFigure keyFig = new TY_KeyFigure(ob_Scrip_BalSheet.getyear(),
							          (ob_Scrip_BalSheet.getDPS() / ob_Scrip_BalSheet.getFaceValue()));
