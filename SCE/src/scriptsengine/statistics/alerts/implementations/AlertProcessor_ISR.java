...
@Service("AlertProcessor_ISR")
public class AlertProcessor_ISR implements IAlertAware
{
	@Autowired
		...
		if (attrContainer != null)
		{
			if (attrContainer.getDeltaFigures() != null)
			{
				...
				if (attrContainer.getDeltaFigures().size() >= timePeriod)
				{
					...
					{
						/** Alert Messages **/
						GeneralMessage msgChgErr = new GeneralMessage("AL_ISRTREND", new Object[]
					...
					if (alertTriggered2 == true)
					{
						...
						GeneralMessage msgChgErr = new GeneralMessage("AL_ISRPENU", new Object[]
						{ penDebt
						});
						alertMsgs.add(new TY_Alert(SCEenums.alertMode.AGAINST, msgFormatter.generate_message_snippet(msgChgErr)));
					}
